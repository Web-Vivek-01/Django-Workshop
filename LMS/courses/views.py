...
class lmsDeleteUserByEmail(APIView):
    def delete(self,request):
        userdata = lmsDeleteUserByEmailSerliazer(data=request.data)
        if userdata.is_valid():
            email = userdata.data["email"]
            lmsUser.objects.filter(email=email).delete()
            message = {"message": "User deleted Successfully"}
            return JsonResponse(message,status=status.HTTP_204_NO_CONTENT)
        return JsonResponse(userdata.errors,
                            status=status.HTTP_400_BAD_REQUEST)
    
# This view deletes a user based on the provided email. It uses the `lmsDeleteUserByEmailSerliazer` to validate the input data. If the data is valid, it deletes the user and returns a success message. If the data is invalid, it returns the validation errors.